...
class TableDecoder(BaseDecoder):
    ...
    def from_stim_circuit(
        ...
        assert data_len <= 64, "can't pack into int"
        ...
        for _ in tqdm(range(progress_bar_steps)):
            ...
            assert isinstance(det_obs_shots, np.ndarray)
    ...
    def from_det_obs_shots(
        ...
        num_detectors = dem.num_detectors
        num_observables = dem.num_observables
        assert isinstance(num_detectors, int)
        assert isinstance(num_observables, int)
    ...
    def det_obs_dataframe(self):  # type: ignore[no-untyped-def]
        ...
        if not self._is_cached_df:
            ...
            keys = [
                f"det-{i}" for i in range(self.num_detectors)
    ...
    def update_det_obs_counts(self, det_obs_shots: np.ndarray) -> None:
        ...
        assert data_len == det_obs_shots.shape[1], "Data length mismatch"
    ...
    def cache_correction(self) -> None:
        ...
        if not self._is_cached_correction:
            ...
            self._maximum_likelihood_correction = np.argmax(
                obs_counts, axis=0
            ).reshape(-1)
    ...
    def _decode(
        ...
        return unpack_boolean_array(
            np.array([correction_idx]), self.num_observables
        )[0]
    ...
    def decode(
        ...
        packed_correction = self._maximum_likelihood_correction[
            packed_det_shots
        ]
        return unpack_boolean_array(
            packed_correction, self.num_observables
        )

    def decode_det_obs_counts(
        self, raw_det_obs_counts: np.ndarray
    ) -> np.ndarray:
        ...
        assert 1 << (num_detectors + num_observables) == (
            raw_det_obs_counts.shape[0]
        ), "Data length mismatch"
        appended_correction = (
            self._maximum_likelihood_correction << num_detectors
        )
        labels = np.arange(
            1 << (num_detectors + num_observables)
        ).reshape(1 << num_observables, 1 << num_detectors)
        repeated_appended_correction = appended_correction.reshape(
            1, -1
        ).repeat(1 << num_observables, axis=0)
        corrected_labels = labels ^ repeated_appended_correction
        decoded_det_obs_counts = raw_det_obs_counts[
            corrected_labels.reshape(-1)
        ]
...
def det_obs_shots_to_counts(
    det_shots: np.ndarray, obs_shots: np.ndarray
) -> np.ndarray:
...
class _CompiledTableDecoder(sinter.CompiledDecoder):
...
class SinterTableDecoder(sinter.Decoder):
