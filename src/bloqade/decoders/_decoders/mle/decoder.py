...
class GurobiDecoder(BaseDecoder):
    ...
    def _solve_single_shot(
        ...
        *,
        ...
        detector_shot = detector_shot.astype(int) ^ self._certain_det_flip
        ...
        if forbidden_logical is not None:
            internal_forbidden = forbidden_logical.astype(
                bool
            ) ^ self._certain_obs_flip.astype(bool)
            diff_variables: list[gp.Var] = []
            for obs_idx, forbidden_bit in enumerate(internal_forbidden.astype(int)):
        ...
        logical ^= self._certain_obs_flip.astype(bool)
