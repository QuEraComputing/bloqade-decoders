...
structural_with_annotate = structural_no_opt.union([annotate])


def test_repeat_typeinfer_single_return_type():

    @structural_with_annotate
    def main():
        def body():
            return 42

        return annotate.repeat(num_iterations=3, method=body)

    passes.Fold(structural_with_annotate).fixpoint(main)
    passes.TypeInfer(structural_with_annotate)(main)
